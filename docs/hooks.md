...
2. A **background scanner** that scans on save with debouncing and fills `.codefence/cache/` so pre-commit is faster.
...
Installed hooks invoke the **`codefence` binary** on `PATH` (or a configured absolute path). Ensure `codefence` is available in the environment where Git and the IDE run hooks.

## Install

From your application repo (with `codefence` on `PATH`):
...
| Path | Purpose |
| ---- | ------- |
| `.git/hooks/pre-commit` | Invokes `codefence pre-commit` on every `git commit` |
| `.kiro/hooks.json` | Kiro `afterFileEdit` → `codefence background-scan` (created if missing) |
...
```bash
codefence install-hooks --dry-run
```
...
```bash
codefence background-scan --file src/example.go
```
...
```bash
echo '{"file_path":"src/main.go"}' | codefence background-scan
```

PowerShell:

```powershell
'{"file_path":"src/main.go"}' | codefence background-scan
```

### Kiro-style env var

```bash
export KIRO_EDITED_FILE=src/main.go
