...
| `tests/depsExtraction.test.ts` | [x] npm, Python, Go, Ruby, PHP, JVM, .NET; lockfiles and merge precedence |
...
2. [x] Staged manifests with extractors trigger OSV query flow (`examples/deps` covers npm, Python, Go, Ruby, PHP, JVM, .NET); Rust, Swift, and remaining lockfiles trigger the aspect but yield no coordinates until parsers land.
...
- [ ] **Dependency extraction** for remaining ecosystems (Rust, Swift, `composer.lock`, `Cargo.lock`, …) — trigger-only or lockfile parsers not shipped
...
- [ ] **Integration tests** per manifest type (extraction + OSV) for Rust/Swift and other trigger-only types (Ruby/PHP/JVM/.NET covered via `tests/depsExamples.test.ts` + `examples/deps`)
...
2. Additional ecosystems and lockfiles (`composer.lock`, Rust, Swift, …) — see [multi-ecosystem-manifest-extraction.md](./multi-ecosystem-manifest-extraction.md)
