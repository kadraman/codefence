...
Use **`--deps-scope tree`** to discover every dependency manifest under the repository (or under each `--paths` directory), including npm, Python, Go, Ruby, PHP, JVM, and .NET files (see [dependency-support.md](docs/dependency-support.md)). Common vendor directories (`node_modules`, `.git`, `.codefence`, etc.) are skipped.
